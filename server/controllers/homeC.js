...
//Make description of the vacancy prettier

const makeTextPrettier = (text) => {
    return `${text.replace(/ {4}|[\t\n\r]/gm, '').slice(0, 130).trim()}...`
}
...
async function createURL(values, site, rules) {
    ...
    switch (site) {
        ...
        case "workUA":
            ...
            if (values[2] != 'all') {   //якщо місто пошуку вказано - йде зміна посилання
                url = `${url}-${values[2]}`;
            }

            if (values[0]) {    //при наявності запиту конкретної вакансії - назва кодується і додається до посилання
                url = `${url}-${encodeURIComponent(values[0])}`;
            }
        ...
        case "jobsUA":
            ...
            if (values[0]) {
                url = `${url}/rabota-${encodeURIComponent(values[0])}`;
            }
        ...
        case "dou":
            ...
            if (values[0]) {
                url = `${url}&search=${encodeURIComponent(values[0])}`;
            }
        ...
        case "jooble":
            ...
            if (values[4]) {
                ...
                if (values[4] == 2)
                    url = `${url}&salaryMin=11000&salaryRate=3`;
            ...
            if (values[0]) {
                url = `${url}&ukw=${encodeURIComponent(values[0])}`;
            }
...
async function getAllData($, path, ptTitle, ptLink, ptSal, ptComp, site, ptDesc) {
    let arr = [];
    ...
    $(path).each((i, el) => {
        arr.push({
            ...
            site: site,
            desc: makeTextPrettier($(el).find(ptDesc).text())
        })
    ...
    switch (site) {
                    ...
                    if (Math.ceil(count / 14) == 1) {
                        await axios
                            ...
                            .then(async function ({ data }) {
                                ...
                                await getAllData($, rules.workUA.path, rules.workUA.pathTitle, rules.workUA.addPathForLinks, rules.workUA.pathSalary, rules.workUA.pathCompany, site, rules.workUA.textPath)
                                    .then(data => {
                                        newArrJobs = data;
                                    })
                            })
                ...
                if (count) {
                    ...
                    if (Math.ceil(count / 20) == 1) {
                        await axios
                            ...
                            .then(async function ({ data }) {
                                const $ = load(data); // Initialize cheerio 
                                await getAllData($, rules.jobsUA.path, rules.jobsUA.pathTitle, rules.jobsUA.addPathForLinks, rules.jobsUA.pathSalary, rules.jobsUA.pathCompany, site, rules.jobsUA.textPath)
                                    .then(data => {
                                        newArrJobs = data;
                                    })
                            })
        ...
        case 'dou':
            ...
            await axios
                ...
                .then(async function ({ data }) {
                    ...
                    await getAllData($, rules.dou.path, rules.dou.pathTitle, rules.dou.addPathForLinks, rules.dou.pathSalary, rules.dou.pathCompany, site, rules.dou.textPath)
                        .then(data => {
                            newArrJobs = data;
                        })
        ...
        case 'jooble':
            ...
            await axios
                ...
                .then(async function ({ data }) {
                    const $ = load(data); // Initialize cheerio 
                    await getAllData($, rules.jooble.path, rules.jooble.pathTitle, rules.jooble.addPathForLinks, rules.jooble.pathSalary, rules.jooble.pathCompany, site, rules.jooble.textPath)
                        .then(data => {
                            newArrJobs = data;
                        })
...
async function getAllVacancies(values) {
    let arr = [];
    await createAllURLs(values, jobsArr)
...
export const getHome = async (req, res) => {
    ...
    await getAllVacancies(values)
        .then(arr => {
            const q = "INSERT INTO parser(`jobTitle`,`jobSalary`,`jobLink`, `jobSite`, `jobCompany`, `jobDesc`) VALUES (?)";
